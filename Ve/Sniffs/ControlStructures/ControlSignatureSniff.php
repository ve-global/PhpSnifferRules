...
class Ve_Sniffs_ControlStructures_ControlSignatureSniff implements PHP_CodeSniffer_Sniff
{
	 ...
	 */
	private function checkSingleSpaceAfterKeyword(array $tokens, $stackPtr, PHP_CodeSniffer_File $phpcsFile)
	{
		if (in_array($tokens[$stackPtr]['code'], [T_TRY, T_DO, T_ELSE])) {
			$this->checkNewLineAfterPointer($tokens, $stackPtr, strtoupper($tokens[$stackPtr]['content']) . ' keyword', $phpcsFile);
		} else {
			$found = 1;
	 ...
	 * @param PHP_CodeSniffer_File $phpcsFile
	 */
	private function checkNewLineAfterClosingParenthesis(array $tokens, $stackPtr, PHP_CodeSniffer_File $phpcsFile)
	{
		if (isset($tokens[$stackPtr]['parenthesis_closer']) === true
            && isset($tokens[$stackPtr]['scope_opener']) === true
        ) {
            $closer  = $tokens[$stackPtr]['parenthesis_closer'];
			$this->checkNewLineAfterPointer($tokens, $closer, 'closing parenthesis', $phpcsFile);
	 ...
	 * @param PHP_CodeSniffer_File $phpcsFile
	 */
	private function checkNewLineAfterOpeningBrace(array $tokens, $stackPtr, PHP_CodeSniffer_File $phpcsFile)
	{
	 ...
	 * @param PHP_CodeSniffer_File $phpcsFile
	 */
	private function checkNewLineAfterPointer(array $tokens, $pointer, $pointerName, PHP_CodeSniffer_File $phpcsFile)
	{
		for ($next = ($pointer + 1); $next < $phpcsFile->numTokens; $next++) {
                ...
                if ($tokens[$next]['line'] === $tokens[$pointer]['line']
            ...
            if ($found !== 1) {
                ...
                $data  = array($found);
                $fix   = $phpcsFile->addFixableError($error, $pointer, 'NewLineAfterControlPointer', $data);
                if ($fix === true) {
