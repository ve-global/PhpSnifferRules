...
class Ve_Sniffs_Classes_AbstractClassNameSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
		...
		if ($tokens[$stackPtr -2]['code'] === T_ABSTRACT && strpos($className, 'Abstract') !== 0)
		{
			$phpcsFile->addError('The abstract class "' . $className . '" does not have the Abstract prefix in its name.', $stackPtr, 'WrongName');
		}
    }
